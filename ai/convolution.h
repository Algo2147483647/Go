...
void img2col(int kernelSize, int channels, vector<float>& in, vector<float>& out) {
    if (kernelSize == 1) {
        copy(begin(in), begin(in) + channels * BOARD_COLOR_NUM * sizeof(float), begin(out));
        return;
    }
    ...
    for (int c = channels; c--; data_im += BOARD_COLOR_NUM) {
...
void convolve_img2col(int w, int h, int inC, int outC,
    ...
    auto C_mat = Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(out.data(), BOARD_COLOR_NUM, outputs);
    C_mat.noalias() =
        Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(col.data(), BOARD_COLOR_NUM, kernelSize * kernelSize * inC) *
        Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(weights.data(), kernelSize * kernelSize * inC, outputs);

    // biases
    for (int o = 0; o < outputs; o++)
        for (int b = 0; b < BOARD_COLOR_NUM; b++)
            out[(o * BOARD_COLOR_NUM) + b] += biases[o];
}
